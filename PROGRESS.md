...
| `buf.gen.yaml:29,31,33` | Unpinned remote plugins — `protocolbuffers/python`, `pyi`, `grpc/python`. The reason the floors are *derived*. Pinning them is **rejected**: `DECISIONS.md:1199`. |
...
- **Verifier:** Opus, fresh agent each round. **Rounds: 3 → PASS.** All findings were
  ...
  - *R2 GAPS (3):* the `:109`→`:141` fix reached `PROGRESS.md` but missed
    ...
    `DECISIONS.md` section titles, which are lookup keys that must match `DECISIONS.md:1391`
    verbatim, not paths.
