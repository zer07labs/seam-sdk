...
| Plan | Status |
|---|---|
| `consumer-decoders-and-event-surface.md` | **DONE** (2026-09-01) — issues [#87](https://github.com/zer07labs/seam-sdk/issues/87), [#88](https://github.com/zer07labs/seam-sdk/issues/88), [#85](https://github.com/zer07labs/seam-sdk/issues/85). Five phases in three PRs: phases 1-2 in [#90](https://github.com/zer07labs/seam-sdk/pull/90), phases 3-4 in [#93](https://github.com/zer07labs/seam-sdk/pull/93), phase 5 in [#94](https://github.com/zer07labs/seam-sdk/pull/94), reconciled in the last of those. (This row said "shipped (#92)" while in progress; there is no PR #92 — the live-server work is #90, verified against `git log origin/main`, which is the kind of claim this file's own header says to check against code rather than a status table.) The decoder half is `policy_enforcement_of` / `policyEnforcementOf`, the presence-aware pair that stops an absent `policy_enforcement` reading as "no policy was enforced"; the contract half is `contract/event-field-manifest.txt`, closing the last unmanifested surface — 90 `seam.event.v1` fields, compared per language in both directions, exit 8. |
