...
- **No widening of the protobuf floor.** It is derived, not chosen; widening means pinning `buf.gen.yaml`'s remote plugins, which `DECISIONS.md:419-439` records as rejected with a stated re-open trigger ("a second framework becomes incompatible") that has **not** fired — a live probe confirms three of four frameworks resolve today.
...
3. ~~With the manifest as committed, `STREAM=1 EVENTS=1 ./scripts/check-contract.sh` exits 0 against the current stubs.~~ — **superseded, see Divergence in the Status note above.** The committed manifest is the BSR's 228, so the gate exits 0 in CI (which regenerates from the BSR first) and **6** in a checkout whose stubs predate ACDP. Exit 6 naming the five fields is the gate working; meeting this criterion as written would have required committing a knowingly-stale 223.
...
8. Both extractors agree: the Python-written manifest set-compares clean against the TypeScript extraction — zero diff in either direction — with no synthetic map-entry residue on either side, and with `ResumeRequest/raise`, `AdminResumeRequest/raise`, `AuditEntry/seq` and `AuditEntry/decision_id` all present. ~~**223 entries** … any count other than 223 means the extractor is wrong, not the manifest.~~ — **superseded:** 223 is the pre-ACDP surface this plan measured; the committed manifest is **228**, matching the BSR. The invariant that survives is *agreement between the two extractors*, not the literal count — the count is a property of the contract on the day it is read.
...
Rejected: pinning `buf.gen.yaml`'s remote plugins for reproducibility. It collides with the already-recorded rejection at `DECISIONS.md:419-439` and freezes the codegen pipeline; (a) and (b) buy reproducibility *of correctness* without that cost.
...
**Status:** DONE — the one existing anchor was **converted**, not grandfathered (the phase permitted either). **Correction, found by the final whole-feature verify:** the reason given at the time — "Phase 9's regeneration half refreshes that same vendored file again" — was counterfactual. That refresh (`c7331b6`, PR #80) landed about two hours *before* this phase, and Phase 9's regeneration commit never touches the file. Converting still stands on the durable reason: the copy is refreshed whole-file on upstream's cadence, so the next refresh drifts any line anchor into it. The choice is **not yet exercised** — the first refresh after this is its test. The converted claim is not merely un-checked: a new line-number-free `QUOTED` mechanism asserts the needle is unique in the target, that `DECISIONS.md` quotes it verbatim, and that the attribution sits within two lines of the quote. That is a **trade, not a superset** — the line-position claim is genuinely dropped, and what is gained is that the document's own words are now checked against the source, which `ANCHORED` never did. Worth taking only because the dropped half is the half that cannot be kept true against a whole-file refresh.
