...
> repointed — but **only after two rounds**: repointing them by hand missed six and re-broke one,
> which round 2 of the gate caught, so the fix was to stop hand-repointing and sweep both documents
> against the current files mechanically. That failure is the finding, not a footnote to it: this is
...
Also add the branch-ancestry assertion that `version-check` (`publish.yml:150-188`) lacks: tags are not branch-scoped and `ci.yml:19` runs on every branch push, so a tag pushed at a green **feature-branch** commit that never reached `main` publishes cleanly today. `git merge-base --is-ancestor "$GITHUB_SHA" origin/main` closes it.
...
*#48.* `crewAIInc/crewAI#7103` — the upstream ask that gates this — **is already filed and open**, and neither `COMPATIBILITY.md:147`'s Tracking cell nor issue #48 links it. Add the link and note its state. The §4a verdict itself is correct today and needs no change: a live resolution probe confirms crewai incompatible and langchain / strands-agents / claude-agent-sdk compatible, matching the committed table. Respect the `<!-- PROBE-TABLE: -->` marker at `:130-132` — columns and order are load-bearing and machine-read.
...
3. `COMPATIBILITY.md:147`'s crewai Tracking cell links `crewAIInc/crewAI#7103`; issue #48 has a comment doing the same.
