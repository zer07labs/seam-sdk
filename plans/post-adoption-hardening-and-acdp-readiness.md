...
### Phase 7 — `COMPATIBILITY.md` pass: document **at least** 0.7.40-0.7.43, cross-link the upstream ask, answer #76
...
**Acceptance criteria.**
1. `COMPATIBILITY.md` §3 contains a row covering **at least** 0.7.40-0.7.43 stating the narrow condition, the root cause, 0.7.47 as the fixed release, and the "may reach back further" hedge (the floor string alone does not bound the band — it dates to 0.7.13; 0.7.40 is the publication cluster's edge, not a proven start); the §3 preamble reads true alongside it.
2. `python/tests/test_retracted_claims.py` parametrize includes `"0.7.43"` and passes.
3. `COMPATIBILITY.md:136`'s crewai Tracking cell links `crewAIInc/crewAI#7103`; issue #48 has a comment doing the same.
...
3. `yank.yml`'s token resolution strips a leading `"Bearer "` from either source, matching `publish.yml:306-307`.
