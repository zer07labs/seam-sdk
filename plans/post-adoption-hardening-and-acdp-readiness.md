...
> **Divergence 1 — the tag count was 27, measured 26.** `protobuf>=7.35.1,<8` is declared
> continuously from `v0.7.13` to `v0.7.43`, but 0.7.22-0.7.25 and 0.7.33 were never tagged, so the
> band spans 26 tags, not 27. The document says 26. The argument is unaffected — the point is that
> the floor string long predates the skew and so cannot mark its start.
>
> **Divergence 2 — `crewAIInc/crewAI#7103` is a pull request, not an issue**, open since
> 2026-08-24. `COMPATIBILITY.md` links it as a PR and says what its merging would mean, which is
> the state the `incompatible` row actually depends on.
>
> **Divergence 3 — the §3 preamble did need amending, as the phase anticipated it might.** Its
> first limb ("a clear auth error rather than corrupting data") does not describe a `VersionError`
> at import, and its second ("a floor already in wide use") does not describe this band at all.
> Rewritten so the shared claim is *fails loudly rather than quietly* — which all three bands
> satisfy — with the wide-use limb explicitly scoped to the first two.
>
> **Divergence 4 — the issue replies are drafted, not posted.** AC3 and AC4 each have two halves;
> the `COMPATIBILITY.md` half is done. The replies on #48 and #76 are held until the PR exists, so
> they can point at a merged paragraph instead of at nothing. Carried into ship, not dropped.
...
*#48.* `crewAIInc/crewAI#7103` — the upstream ask that gates this — **is already filed and open**, and neither `COMPATIBILITY.md:191`'s Tracking cell nor issue #48 links it. Add the link and note its state. The §4a verdict itself is correct today and needs no change: a live resolution probe confirms crewai incompatible and langchain / strands-agents / claude-agent-sdk compatible, matching the committed table. Respect the `<!-- PROBE-TABLE: -->` marker at `:185-187` — columns and order are load-bearing and machine-read.
...
3. `COMPATIBILITY.md:191`'s crewai Tracking cell links `crewAIInc/crewAI#7103`; issue #48 has a comment doing the same.
