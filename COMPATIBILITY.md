...
`seam-adapters` publishes a runtime × SDK × adapters matrix and asked ([#76](https://github.com/zer07labs/seam-sdk/issues/76)) what a
cell means, so that the three repos do not publish contradictory ones. This is the definition this
repo holds to, and it is deliberately the weaker of the two available readings.
...
**A `compatible` matrix cell asserts resolution plus one executed lane — and nothing about the wire
contract.** It means: these versions resolved together, and a live gRPC lane ran against a runtime
on that combination. Assert it from a run where that lane **actually executed** — in this repo the
live-gRPC job (`integration`) is *advisory*: it may skip and must not fail
(`scripts/test_ci_gate.py:52`), so a green pipeline is not by itself evidence the lane ran. It does
**not** assert wire-contract equivalence. Under §1's lockstep
rule the only thing that establishes *that* is the conformance vectors, which are generated by
executing the runtime's reference implementation rather than by agreeing with our own tests. So a
cell is a narrower claim than "these versions agree on the protocol", on purpose: the stronger
reading is one no per-cell check in this repo can actually support, and a matrix that overstates
what it verified is worse than one that admits a gap. Anything CI did not execute is `untested`,
never blank — a blank cell reads as "fine". `seam-adapters` adopted that rule after retracting a
false capability claim of its own; this repo has its own retraction history for related reasons
(`python/tests/test_retracted_claims.py`), and agrees with the rule.

**A floating cell should record the digest, not the tag.** Where a lane pulls a `:main` image and
resolves the SDK range at run time, neither end has a version string worth citing — a tag that
moves is not an identifier, and a cell naming one asserts something that stops being true without
any edit. Record instead what the run already knows: the **image digest** (`sha256:…`) actually
pulled, and the **exact SDK version the resolver picked**. That turns "resolved at run time" into a
cell that names what was tested without claiming it was a release. This SDK publishes no
compatibility statement that supersedes the **0.7.20** floor, so cite the floor and §3's bands —
there is nothing stronger of ours to wait for.
...
`CHANGELOG.md:610-615` and is not being re-litigated: they fail loudly — an unimportable wheel, or a
...
| **0.7.39 – 0.7.43** | `import seam_sdk` raises `VersionError` naming both versions — but **only** for a consumer whose dependency closure caps `protobuf` below 7.36.0. Installation resolves cleanly either way, so nothing fails until the first import. | The wheel declares `protobuf>=7.35.1,<8` over gencode **7.36.0**, and protobuf's `ValidateProtobufRuntimeVersion` rejects a runtime *older* than the gencode that produced the stubs. `buf.gen.yaml` pins no plugin version, so a remote-plugin roll moved the gencode under a floor nobody re-derived ([#52](https://github.com/zer07labs/seam-sdk/issues/52)). Closed by PR #51 (`f68572f`). **Fixed in 0.7.47.** |

**Floor: 0.7.20.** The first release that is both importable and wire-correct.

**Both edges of the third band are proven, and the evidence is CI's rather than the repo's.** No
per-tag gencode survives in the tree — the stubs are gitignored — so it is tempting to call the
lower edge unrecoverable. It is not: every tagged commit has a CI run, and
`test_the_declared_floor_is_at_least_the_gencode_in_the_generated_stubs` *is* this defect.
`v0.7.38` (2026-08-20T19:47Z) is **green**; `v0.7.39` (2026-08-22T06:39Z) is the first **red**, and
0.7.40 through 0.7.43 are red the same way. `f68572f` returns it to green for 0.7.47.

What moved at that boundary was the **buf remote plugin**, not the protobuf runtime: v0.7.38's own
green run already installed protobuf 7.36.0 as its runtime, and stayed green because the *gencode*
was still 7.35.x. Runtime and gencode roll independently, which is precisely why a derived floor
can go stale with nothing looking wrong.

**Five consecutive releases published on red CI** — 0.7.39, 0.7.40, 0.7.41, 0.7.42 and 0.7.43 —
each red on this exact test, over four days. [#52](https://github.com/zer07labs/seam-sdk/issues/52) names only the last of them. That is the
number worth remembering: the gate existed and was failing the whole time, and `publish.yml` never
consulted it.

None of the three defects is visible from the version number. That is the lockstep rule in §1 doing exactly what
it says it does.
...
publish job's own generation (`.github/workflows/publish.yml:354`), and the built wheel is then
installed into a venv with `protobuf` pinned at the floor it declares and imported there
(`.github/workflows/publish.yml:428`) — the only check that answers *is this metadata true?*, since
...
comment — `.github/workflows/ci.yml:436-443` runs `cargo tree -e normal` and fails on any `seam-*`. What it verifies:
